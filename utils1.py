...
def rescale_frame(frame, percent=50):
    """Rescale a frame from OpenCV to a certain percentage compared to its original frame
...
def save_frame_as_image(frame, message: str = None):
    """
    Save a frame as an image to display the error
    """
...
def get_static_file_url(file_name: str) -> str:
    """Return a static URL of a file

    Args:
        file_name (str)

    Returns:
        str: Full absolute path of the file. Return None if the file is not found
    """

    path = f"/path/to/static/files/{file_name}"
    print(path)
