...
from process_frame_bicep import BicepPoseAnalysis  
...
def process_video1(mode):
    ...
    if mode == 'Live Stream':
        ...
        mode = st.radio('select side',['Left','Right'], horizontal=True)

        if mode=='Left':
            live_process_frame = BicepPoseAnalysis(
    side="left",  # Specify the side ("left" or "right")
    stage_down_threshold=120,  # Example threshold values
    ...
    visibility_threshold=0.65,
            ...
            live_process_frame = BicepPoseAnalysis(
    side="right",  # Specify the side ("left" or "right")
    stage_down_threshold=120,  # Example threshold values
    stage_up_threshold=100,
    peak_contraction_threshold=60,
    loose_upper_arm_angle_threshold=40,
    visibility_threshold=0.65,
        ...
        upload_process_frame = BicepPoseAnalysis(
    side="left",  # Specify the side ("left" or "right")
    stage_down_threshold=120,  # Example threshold values
    stage_up_threshold=100,
    peak_contraction_threshold=60,
